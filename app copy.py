...
def get_dashboard_data():
    """Fetch dashboard data with active instance filter"""
    conn = get_central_conn()
    query = """
        SELECT * FROM v_EnhancedDashboard 
        WHERE IsActive = 1
    """
    return pd.read_sql(query, conn)

def get_health_summary():
    """Fetch health summary for active instances only"""
    conn = get_central_conn()
    query = """
        SELECT * FROM v_InstanceHealthSummary 
        WHERE IsActive = 1
    """
    return pd.read_sql(query, conn)
...
with st.sidebar.expander("➕ Add New Instance", expanded=False):
    with st.form("add_form", clear_on_submit=True):
        ...
        new_svr = st.text_input("Server IP", placeholder="10.xxx.xxx.xxx\InstanceName")
        new_hostname = st.text_input("Hostname", placeholder="LKELSFETEGTRPT")
    ...
    if dashboard_df.empty:
        ...
        with col_left:
            ...
            if not health_summary.empty:
                ...
                # Handle NULL values
                health_display['AvgJobDuration'] = health_display['AvgJobDuration'].fillna(0).round(2)
                health_display['LastDataCollection'] = health_display['LastDataCollection'].fillna('Never')
    ...
    if instances_list.empty:
        ...
        with col2:
            jobs_list = pd.read_sql(
                f"SELECT DISTINCT JobName FROM JobLogs WHERE ServerName = ? ORDER BY JobName",
                conn,
                params=[selected_instance]
            )
            
            if not jobs_list.empty:
                selected_job = st.selectbox("Select Job", options=jobs_list['JobName'].tolist())
            else:
                st.warning("No jobs found for this instance")
                selected_job = None
        ...
        if selected_instance and selected_job:
            job_history = pd.read_sql(f"""
                ...
                FROM JobLogs
                WHERE ServerName = ? AND JobName = ?
                ORDER BY LastRun DESC
            """, conn, params=[selected_instance, selected_job])
            
            if not job_history.empty:
                # Handle NULLs
                job_history['DurationSeconds'] = job_history['DurationSeconds'].fillna(0)
                job_history['CPUTimeMS'] = job_history['CPUTimeMS'].fillna(0)
                job_history['ErrorMessage'] = job_history['ErrorMessage'].fillna('')
...
with tab4:
    ...
    if not perf_trends.empty:
        ...
        selected_instances_perf = st.multiselect(
            ...
            default=perf_trends['FriendlyName'].unique()[:min(3, len(perf_trends['FriendlyName'].unique()))]
        )
        
        if selected_instances_perf:
